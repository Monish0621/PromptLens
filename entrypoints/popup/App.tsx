...
export default function App() {
  ...
  return (
    <div className="w-[390px] bg-slate-900 text-slate-100 p-4 font-sans select-none flex flex-col gap-4 border border-slate-800 shadow-2xl rounded-xl max-h-[600px] overflow-y-auto scrollbar-thin">
      ...
      {/* ── Multi-tab Injection Selector ── */}
      {pendingInjection && (
        <div className="border border-indigo-500/30 bg-indigo-950/30 rounded-xl p-3 flex flex-col gap-2.5 shadow-inner">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <CornerDownLeft className="w-3.5 h-3.5 text-indigo-400" />
              <span className="text-xs font-bold text-slate-200">Send to</span>
            </div>
            <button
              onClick={() => {
                setPendingInjection(null);
              ...
              className="text-[9px] font-semibold text-slate-500 hover:text-slate-300 transition cursor-pointer"
            >
              Dismiss
            </button>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            {pendingInjection.candidates.map(tab => {
              const checked = selectedTabIds.has(tab.tabId);
              const result = injectResults[tab.tabId];
              return (
                <label
                  key={tab.tabId}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-semibold text-slate-200 truncate">{tab.name}</div>
                    <div className="text-[9px] text-slate-500 truncate">{tab.title}</div>
                  </div>
                  {result === 'success' && <Check className="w-3.5 h-3.5 text-emerald-400 shrink-0" />}
                  {result === 'fail' && <AlertCircle className="w-3.5 h-3.5 text-rose-400 shrink-0" />}
                  {result === 'pending' && <span className="w-3.5 h-3.5 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin shrink-0" />}
                </label>
              );
            })}
          </div>
          ...
          <button
            ...
            className="w-full py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs font-bold transition flex items-center justify-center gap-1.5 shadow-lg shadow-indigo-600/20 cursor-pointer"
          >
            <CornerDownLeft className="w-3.5 h-3.5" />
            {isDispatching ? 'Injecting…' : `Inject into ${selectedTabIds.size} tab${selectedTabIds.size !== 1 ? 's' : ''}`}
          </button>
        </div>
      )}
